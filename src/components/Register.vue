    ...
    methods: {
      async handleSubmit() {
        try {
          ...
          this.message = response.data.message; // Adjust based on your response structure
        } catch (error) {
          this.message = 'An error occurred: ' + (error.response ? error.response.data.message : error.message);
        }
      }
    }
  };
